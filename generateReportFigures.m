...
%GENERATEREPORTFIGURES Create the detailed per-level case-study figures.
%   Regenerates the Level 1 calibration, Level 3 inverse-optimal-control, and
%   Level 4 inverse-differential-game detail figures from the saved
%   inverse_ladder_results.mat. The PNGs are written next to the results file
%   (the same folder that holds inverse_ladder_summary.png); copy them into the
%   manuscript figures folder as needed. Run runInverseLadderCaseStudy first so
%   that inverse_ladder_results.mat exists.

codeFolder = fileparts(mfilename("fullpath"));
figureFolder = codeFolder;
addpath(codeFolder);

loaded = load(fullfile(codeFolder, "inverse_ladder_results.mat"), ...
    "resultsToSave");
...
nexttile;
% Color encodes true vs inferred (blue/orange, matching Levels 1 and 3 and the
% summary figure); opacity encodes the player (solid = P1, translucent = P2).

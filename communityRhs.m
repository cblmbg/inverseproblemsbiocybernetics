...
% Clamp genuine out-of-range values. A NaN is left in place so it propagates
% and is detected by the caller rather than being masked; note that a saturating
% clamp still maps an out-of-range infinity onto the nearest bound. Callers
% validate finite states, controls, and parameters before integrating (see
% simulateControlledModel, simulateCommunity, validateModelParameters).
x = x(:);
x(x < 0) = 0;
u = u(:);
u(u < 0) = 0;
